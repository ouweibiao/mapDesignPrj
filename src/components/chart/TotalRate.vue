...
async function fetchData() {
    try {
        const filePath = `../../../store/indicator/global/global${currentIndex.value}.json`;
        const data = await import(/* @vite-ignore */ filePath);
        chartData.value.push(data.default[0][2]);
...
function updateChart() {
    const option = {
        ...
        series: [
            {
                data: chartData.value, // 使用数据中的某一字段作为 Y 轴数据
                type: "line",
                smooth: true,
            },
        ],
...
<template>
    <div class="chartbox">
        <h4 class="title center">平均无干扰通行率 当前：{{ chartData[props.currentIndex] == null ? null :
            chartData[props.currentIndex].toFixed(3) }}</h4>
