...
const drawTrace = () => {
    ...
    while (index <= currentIndex.value) {
        const promise = import(`../../store/indicator/vehicle/vehicle${index}.json`).then(data => {
...
const startUpdating = () => {
    if (!intervalId) {
        intervalId = setInterval(() => {
            ...
            // 按照当前索引加载数据并更新地图
            import(`../../store/indicator/vehicle/vehicle${currentIndex.value}.json`).then(data => {
...
<template>
    <div class="center controller">
        <div class="btn" @click="startUpdating"><svg class="icon fonts" aria-hidden="true">
                <use xlink:href="#icon-weibiaoti518"></use>
            </svg>
        </div>
        <div class="btn" @click="pauseUpdating"><svg class="icon fonts" aria-hidden="true">
                <use xlink:href="#icon-iconstop"></use>
            </svg>
        </div>
        <div class="btn" @click="restart"><svg class="icon fonts" aria-hidden="true">
                <use xlink:href="#icon-line-restartzhongxinkaishi"></use>
            </svg>
        </div>
    </div>
